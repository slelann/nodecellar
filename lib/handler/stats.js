...
*/
	...
	handler.renderCountryStats  = function (req, res) {
			...
			db.getWinesByCountry(function(results) {
				var countriesStats = objToArray(results.data,'sum');
				var countriesNames = objToArray(results.data,'_id');
				res.send({'countriesStats': countriesStats, 'countries': countriesNames});
			});
	...
	handler.renderTypeStats  = function (req, res) {
			...
			db.getWinesByType(function(results) {
				var typeStats = objToArray(results.data,'sum');
				var typeNames = objToArray(results.data,'_id');
				res.send({'typeStats': typeStats, 'types': typeNames});
			});
	...
	handler.renderVintageStats  = function (req, res) {
			...
			db.getWinesByVintage(function(results) {
				var vintageStats = objToArray(results.data,'sum');
